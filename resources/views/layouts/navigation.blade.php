<nav x-data="{ open: false }" style="background:#111827; border-bottom:1px solid #1f2d4a; position:relative; z-index:10;">
        ...
        <div class="flex justify-between h-16">
                ...
                <div class="hidden sm:flex gap-6">
                    <a href="{{ route('carros.index') }}"
                       style="font-size:12px; letter-spacing:1px; text-transform:uppercase; text-decoration:none;
                              color: {{ request()->routeIs('carros.*') ? '#4d70b8' : '#8892b0' }};
                              border-bottom: {{ request()->routeIs('carros.*') ? '2px solid #4d70b8' : '2px solid transparent' }};
                              padding-bottom:4px; transition:color 0.2s;">
                        <i class="fas fa-car mr-1"></i> Carros
                    </a>
                    <a href="{{ route('usuarios.lista') }}"
                       style="font-size:12px; letter-spacing:1px; text-transform:uppercase; text-decoration:none;
                              color: {{ request()->routeIs('usuarios.*') ? '#4d70b8' : '#8892b0' }};
                              border-bottom: {{ request()->routeIs('usuarios.*') ? '2px solid #4d70b8' : '2px solid transparent' }};
                              padding-bottom:4px; transition:color 0.2s;">
            ...
            <div class="hidden sm:flex items-center">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button style="display:flex; align-items:center; gap:8px; background:transparent; border:1px solid #1f2d4a; border-radius:6px; padding:8px 14px; color:#8892b0; font-size:13px; cursor:pointer; transition:all 0.2s;"
                                onmouseover="this.style.borderColor='#4d70b8'; this.style.color='#fff'"
                                onmouseout="this.style.borderColor='#1f2d4a'; this.style.color='#8892b0'">
                            <i class="fas fa-user-circle" style="color:#4d70b8;"></i>
                            {{ Auth::user()->name }}
                    ...
                    <x-slot name="content">
                        <div style="background:#111827; border:1px solid #1f2d4a; border-radius:8px; overflow:hidden; min-width:180px;">
                            ...
                            <a href="{{ route('profile.edit') }}"
                               style="display:block; padding:10px 16px; font-size:12px; letter-spacing:1px; text-transform:uppercase; color:#8892b0; text-decoration:none; transition:all 0.2s;"
                               onmouseover="this.style.color='#ffffff'; this.style.background='rgba(77,112,184,0.1)'"
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                        style="width:100%; text-align:left; padding:10px 16px; font-size:12px; letter-spacing:1px; text-transform:uppercase; color:#dc3545; background:transparent; border:none; cursor:pointer; transition:background 0.2s;"
    ...
    <div :class="{'block': open, 'hidden': !open}" class="hidden sm:hidden"
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" style="padding:10px 0; font-size:12px; letter-spacing:1px; text-transform:uppercase; color:#dc3545; background:transparent; border:none; cursor:pointer;">
                <i class="fas fa-sign-out-alt mr-2"></i> Sair
            </button>
        </form>
    </div>
